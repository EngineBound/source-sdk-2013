...
char const* CBaseShader::GetParamName(int paramIndex) const
{
	Assert(paramIndex < NUM_SHADER_MATERIAL_VARS);
	return s_StandardParams[paramIndex].m_pName;
}

char const* CBaseShader::GetParamHelp(int paramIndex) const
{
	Assert(paramIndex < NUM_SHADER_MATERIAL_VARS);
...
char const* CBaseShader::GetParamDefault(int paramIndex) const
{
	Assert(paramIndex < NUM_SHADER_MATERIAL_VARS);
...
void CBaseShader::DrawElements(IMaterialVar** ppParams, int nModulationFlags, IShaderShadow* pShaderShadow, IShaderDynamicAPI* pShaderAPI,
...
{
	IMaterialVar* pTextureVar = s_ppParams[nTextureVar];
	ITexture* pTextVal = pTextureVar->GetTextureValue();
	if (pOutWidth)
		*pOutWidth = pTextVal->GetMappingWidth();
	if (pOutHeight)
		*pOutHeight = pTextVal->GetMappingHeight();
}
...
bool CBaseShader::TextureIsTranslucent( int textureVar, bool isBaseTexture )
{
	...
	if (params[textureVar]->GetType() == MATERIAL_VAR_TYPE_TEXTURE)
	{
		...
		else
		{
			...
			bool bUsingBaseTextureAlphaForSelfIllum = bHasSelfIllum;
